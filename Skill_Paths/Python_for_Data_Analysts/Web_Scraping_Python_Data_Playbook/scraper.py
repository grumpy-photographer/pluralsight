...
def process_car_blocks(soup):
    """Extract info from repeated divs"""
    ...
    with open("./Data/scraped_cars.csv", "w") as f:
...
def extract_data(cb):
    ...
    assert (
        acceleration > 0
    ), f"Expecting acceleration to be positive, not {acceleration}"
    mpg = extract_mpg(cb)
...
if __name__ == "__main__":
    filename = "./Data/scraped_page_result.pickle"
