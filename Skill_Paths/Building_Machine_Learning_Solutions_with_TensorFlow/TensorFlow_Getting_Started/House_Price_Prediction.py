...
tf_cost = tf.reduce_sum(input_tensor=tf.pow(tf_price_pred - tf_price, 2)) / (
    2 * num_train_samples
)
...
with tf.compat.v1.Session() as sess:
    ...
    # calcluate the number of lines to animation
    ...
    training_cost = sess.run(
        tf_cost,
        feed_dict={tf_house_size: train_house_size_norm, tf_price: train_price_norm},
    )
    print(
        "Trained cost=",
        training_cost,
    ...
    plt.plot(
        train_house_size_norm * train_house_size_std + train_house_size_mean,
    ...
    # plot another graph that animates how gradient descent sequentually adjusted size_factor and price_offset to find the values that returned the "best" fit line
    fig, ax = plt.subplots()
    ...
    def animate(i):
        ...
        line.set_ydata(
            (fit_size_factor[i] * train_house_size_norm + fit_price_offsets[i])
            * train_price_std
            + train_price_mean
        )
        return (line,)
    ...
    ani = animation.FuncAnimation(
        fig,
        animate,
        frames=np.arange(0, fit_plot_idx),
        init_func=initAnim,
        interval=1000,
        blit=True,
    )
