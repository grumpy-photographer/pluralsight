...
def test_task2_module3():
    ...
    assert (
        self_iter_e_found
    ), "Did you assign `self.iter_e` to the iterator `iter(self.expenses)`?"
    assert (
        self_iter_o_found
    ), "Did you assign `self.iter_o` to the iterator `iter(self.overages)`?"
...
def test_task4_module3():
                    ...
                    for y in x.body:
                        if (
                            ...
                            if isinstance(y.body[0], ast.Try):
                                ...
                                if (
                                    isinstance(y.body[0].handlers[0], ast.ExceptHandler)
                                    and y.body[0].handlers[0].type.id == "StopIteration"
                                    and y.body[0].handlers[0].name == "stop"
                                ):
    ...
    assert (
        iter_e_next_found
    ), "Inside the try block, did you call `__next__()` on `self.iter_e`?"
    assert (
        exception_handler_found
    ), "Do you have an `except` block that defines `Stopiteration as stop`?"
    assert (
        iter_o_next_found
    ), "Inside the except block, did you call `__next__()` on `self.iter_o`?"
...
def test_task5_module3():
    ...
    assert (
        for_mybudgetlist_found
    ), "Did you create a for loop that iterates `myBudgetList`, with `entry` as the name of the iterator?"
...
def test_task7_module3():
    ...
    try:
        for x in load_ast_tree("budget/BudgetList.py").body:
            if isinstance(x, ast.FunctionDef) and x.name == "main":
                for y in x.body:
                    if isinstance(y, ast.Assign) and isinstance(
                        y.targets[0], ast.Tuple
                    ):
                        if (
                            y.targets[0].elts[0].id == "fig"
                            and y.targets[0].elts[1].id == "ax"
                        ):
    ...
    assert (
        fig_ax_tuple_found and plt_subplots_call_found
    ), "Did you call `plt.subplots()` and assign it a Tuple `fig,ax`??"
...
def test_task8_module3():
            ...
            if isinstance(x, ast.FunctionDef) and x.name == "main":
                for y in x.body:
                    if (
                        ...
                        and isinstance(y.targets[0], ast.Name)
                        and y.targets[0].id == "labels"
                    ):
                        assign_labels_found = True
                        if (
                            y.value.elts[0].s == "Expenses"
                            and y.value.elts[1].s == "Overages"
                            and y.value.elts[2].s == "Budget"
                        ):
                            correct_labels = True
    ...
    assert (
        assign_labels_found and correct_labels
    ), "Did you assign a variable named `labels` to `['Expenses', 'Overages', 'Budget']`?"
...
def test_task9_module3():
            ...
            if isinstance(x, ast.FunctionDef) and x.name == "main":
                for y in x.body:
                    if (
                        ...
                        if (
                            y.value.elts[0].value.id == "myBudgetList"
                            and y.value.elts[0].attr == "sum_expenses"
                            and y.value.elts[1].value.id == "myBudgetList"
                            and y.value.elts[1].attr == "sum_overages"
                            and y.value.elts[2].value.id == "myBudgetList"
                            and y.value.elts[2].attr == "budget"
                        ):
                            correct_values = True
    ...
    assert (
        assign_values_found and correct_values
    ), "Did you assign a variable named `values` to `[myBudgetList.sum_expenses, myBudgetList.sum_overages, myBudgetList.budget]`?"
...
def test_task10_module3():
    ...
    try:
        for x in load_ast_tree("budget/BudgetList.py").body:
            if isinstance(x, ast.FunctionDef) and x.name == "main":
                for y in x.body:
                    if isinstance(y, ast.Expr) and isinstance(y.value, ast.Call):
                        if (
    ...
    assert (
        correct_values
    ), "Did you call `ax.bar()` with the following parameters: `labels, values, color=['green','red','blue']`?"
...
def test_task11_module3():
                ...
                for y in x.body:
                    if isinstance(y, ast.Expr) and isinstance(y.value, ast.Call):
                        if (
                            hasattr(y.value.func, "value")
                            and y.value.func.value.id == "ax"
                            and y.value.func.attr == "set_title"
                        ):
    ...
    assert (
        correct_value
    ), "Did you call `ax.bar()` with the following parameter: `'Your total expenses vs. total budget'`?"
