...
def test_task1_module2():
    ...
    assert path.exists(
        "budget/BudgetList.py"
    ), "Did you create a file named `BudgetList.py` in the `budget/` folder?"
...
def test_task2_module2():
                ...
                if x.name == "BudgetList":
                    for y in x.body:
                        if isinstance(y, ast.FunctionDef) and y.name == "__init__":
                            ...
                            if (
                                "self:sum_expenses:0" in child_assign
                                and "self:expenses" in child_assign
                            ):
                                expenses_assign_found = True
                            if (
                                "self:sum_overages:0" in child_assign
    ...
    assert (
        args_found
    ), "The `__init__` method should take `self` and `budget` as parameters."
    assert (
        self_budget_assign_found
    ), "Did you initialize `self.budget` to `budget` in `__init__`()?"
    assert (
        expenses_assign_found
    ), "Did you initialize `sum_expenses` to 0 and `expenses` to an empty list in `__init__`()?"
    assert (
        overages_assign_found
    ), "Did you initialize `sum_overages` to 0 and `overages` to an empty list in `__init__`()?"
...
def test_task4_module2():
                            ...
                            for z in y.body:
                                ...
                                if isinstance(z, ast.If):
                                    ...
                                    if isinstance(z.test.ops[0], ast.Lt):
                                        lt_found = True
                                    if (
                                        z.test.comparators[0].value.id == "self"
                                        and z.test.comparators[0].attr == "budget"
                                    ):
                                    ...
                                    for item in z.body:
                                        aug_assign = (
                                            utils.get_augassignments_from_child(item)
                                        )
                                        assign = utils.get_assignments_from_child(item)
                                        func_call = utils.get_calls_from_child(item)

                                        if (
                                            "self:sum_expenses:item" in aug_assign
                                            or "self:sum_expenses:self:sum_expenses:item"
                                            in assign
                                        ):
    ...
    assert (
        self_found
        and sum_expenses_found
        and op_add_found
        and lt_found
        and self_budget_found
    ), "Add an `if` statement that checks if `self.sum_expenses+item < self.budget`."
    assert (
        exp_append_item_found
    ), "Inside the if statement, did you call `self.expenses.append(item)`?"
    assert (
        sum_exp_inc_found
    ), "Inside the if statement, did you call `self.sum_expenses += item`?"
...
def test_task5_module2():
                            ...
                            for z in y.body:
                                ...
                                if isinstance(z, ast.If):
                                    for item in z.orelse:
                                        ...
                                        if (
                                            "self:sum_overages:item" in aug_assign
                                            or "self:sum_overages:self:sum_overages:item"
                                            in assign
                                        ):
    ...
    assert append_def_found, "Did you define the method `def append(self, item)`?"
    assert (
        over_append_item_found
    ), "Inside the else statement, did you call `self.overages.append(item)`?"
    assert (
        sum_over_inc_found
    ), "Inside the else statement, did you call `self.sum_overages+=item`?"
...
def test_task7_module2():
    ...
    assert (
        budgetlist_assign_found
    ), "Did you define a `myBudgetList` variable assigned to `BudgetList(1200)`?"
...
def test_task9_module2():
    ...
    assert (
        expenses_assign_found
    ), "Did you define an `expenses` variable assigned to `Expense.Expenses()`?"
    assert (
        read_expenses_call_found
    ), "Did you call `read_expenses('data/spending_data.csv')` on `expenses`?"
...
def test_task10_module2():
    ...
    assert (
        for_expenses_found
    ), "Did you create a for loop that iterates `expenses.list`?"
...
def test_task11_module2():
    ...
    assert (
        print_call_found
    ), "Did you print `'The count of all expenses: '` concatenated with `str(len(myBudgetList))`?"
...
def test_task12_module2():
    ...
    assert (
        main_call_found
    ), 'Did you add a conditional that checks if `__name__ == "__main__"` that runs `main()`?'
