...
def test_task2_module5():
    ...
    for x in calls:
        if expected_timeit_str in x.replace(" ", "") or expected_timeit_str_2.replace(
            " ", ""
        ) in x.replace(" ", ""):
...
def test_task6_module5():
    ...
    ), "Did you assign a Tuple `fig,ax` to a call to `plt.subplots()`?"
...
def test_task7_module5():
            ...
            if isinstance(x, ast.FunctionDef) and x.name == "main":
                for y in x.body:
                    if (
                        ...
                        if (
                            y.value.elts[0].s == "Necessary"
                            and y.value.elts[1].s == "Food"
                            and y.value.elts[2].s == "Unnecessary"
                        ):
                            correct_labels = True
    ...
    ), "Did you assign a variable named `labels` to `['Necessary', 'Food', 'Unnecessary']`?"
...
def test_task8_module5():
    ...
    assert (
        "divided_expenses_sum" in assigns
    ), "Did you create a variable `divided_expenses_sum` assigned to an empty list?"
...
def test_task9_module5():
    ...
    assert (
        found_for_loop
    ), "Did you create a `for` loop that has an iterator called `category_exps` and loops through `divided_set_comp`?"
    assert (
        for_str in fors
...
def test_task10_module5():
    ...
    assert (
        correct_values
    ), "Did you call `ax.pie()` with the following parameters: `divided_expenses_sum, labels=labels, autopct='%1.1f%%'`?"
