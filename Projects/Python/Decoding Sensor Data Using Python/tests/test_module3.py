...
def test_temperature_convert_loop_module3(parse):
    ...
    assert test_code, """Inside your loop, are you converting `rec` value to integer `base=10`
...
def test_sensor_app_temp_info_by_area_module3(parse):
    ...
    my_file_import = my_file.from_imports("temperature_info", "TemperatureData")
    assert (
        my_file_import
    ), "Are you importing `{0}` from `temperature_info` in `{}`".format(test_file)
    ...
    assert test_code, """Are you creating an instance of the '{}' class called `temperature_data`?
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_area` from the `temperature_data` object?
...
def test_sensor_app_temp_info_by_date_module3(parse):
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_date` from the `temperature_data` object?
