...
def test_house_info_create_class_module2(parse):
    ...
    assert (
        my_class.exists()
    ), "Have you created a class called `{0}` in the `{1}` file?".format(
        test_class, test_file
    )

    my_method = my_file.class_(test_class).method(test_method)
    assert my_method.exists(), "Are you defining a constructor called `{}`?".format(
        test_method
    )
    ...
    assert my_class_arguments, """Are you defining a constructor `{0}` for the `{1}` class?
...
def test_house_info_get_data_by_area_loop_module2(parse):
    ...
    assert test_code, """Do you have a `for` loop, looping through `self.data`? 
...
def test_house_info_get_data_by_date_module2(parse):
    ...
    house_info_import = my_file.from_imports("datetime", "date")
    ...
    assert my_class_arguments, """Are you defining a method `get_data_by_date` 
...
def test_house_info_get_data_by_date_loop_module2(parse):
    ...
    assert first_for, """Do you have a `for` loop, looping through `self.data`? 
        Is the current loop value called `record`?"""
    ...
    assert filter_recs, """Are you filtering the data using an `if` statement? 
...
def test_sensor_app_house_info_by_area_module2(parse):
    ...
    assert test_code, """Are you creating an instance of the class `HouseInfo` with 
    ...
    assert test_code, """Are you creating a variable called `test_area` and setting it to 1?
...
def test_sensor_app_house_info_by_date_module2(parse):
    ...
    my_file_import = my_file.from_imports("datetime", "datetime")
    ...
    assert test_code, """Are you creating an instance of the `datetime` class called `test_date` 
    ...
    assert test_code, """Are you creating a variable `recs` and setting it to `house_info.get_data_by_date()`?
          Are you passing `"id"` as the first argument to the method?
