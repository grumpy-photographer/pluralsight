...
def test_energy_create_class_module5(parse):
    ...
    assert test_code, """Are you declararing a constant `ENERGY_PER_BULB`?
...
def test_energy_get_energy_method_module5(parse):
    ...
    assert test_code, """Are you converting `rec` as an `int()` with `base=16`?
    ...
    test_code = (
        ...
        .match({"type": "Return", "value_type": "Name", "value_id": "energy"})
        .exists()
    )
    assert test_code, """Are you returning `energy` from the `{}` method?""".format(
        test_method
    )
...
def test_energy_convert_method_module5(parse):
    ...
    assert test_code, """Inside your loop, are you converting `rec` value through `self._rec_energy()` method?
...
def test_energy_by_area_and_date_methods_module5(parse):
    ...
    assert test_code, """Are you creating a variable called `recs` set equal to 
        ...
        Are you passing "energy" as the only argument to the method call?""".format(
...
def test_energy_calculate_usage_method_module5(parse):
    ...
    assert test_code, """Are you declaring a variable called `total_energy`?
    ...
    test_code = (
        my_method.returns_call()
        .match({"type": "Return", "value_type": "Name", "value_id": "total_energy"})
        .exists()
    )
    assert (
        test_code
    ), """Are you returning a `total_energy` from the `{}` method?""".format(
        test_method
    )
...
def test_sensor_app_energy_info_by_area_module5(parse):
    ...
    my_file_import = my_file.from_imports("energy_info", "EnergyData")
    assert (
        my_file_import
    ), "Are you importing `EnergyData` from `energy_info` in `{}`".format(test_file)
    ...
    assert test_code, """Are you creating an instance of the '{}' class called `energy_data`
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_area` from the `energy_data` object?
    ...
    assert test_code, """Are you setting `total_energy` to the method call `calculate_energy_usage` from the `energy_data` object?
...
def test_sensor_app_energy_info_by_date_module5(parse):
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_date` from the `energy_data` object?
    ...
    assert test_code, """Are you setting `total_energy` to the method call `calculate_energy` from the `energy_data` object?
        Are you passing `data=recs` as the only argument to the method?
        """
