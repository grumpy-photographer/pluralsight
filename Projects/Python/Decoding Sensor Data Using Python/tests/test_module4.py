...
def test_humidity_create_class_module4(parse):
    ...
    my_file_import = my_file.from_imports("house_info", "HouseInfo")
    assert (
        my_file_import
    ), "Are you importing `HouseInfo` from `house_info` in `{}` file".format(test_file)

    my_class = my_file.query("class {0}({1}): ??".format(test_class, parent_class))
    assert my_class.exists(), """Have you created a class called `{0}`?
        Is your class inheritings the properties of the `{1}` class?""".format(
        test_class, parent_class
    )

    my_method = my_file.class_(test_class).method(test_method)
    assert my_method.exists(), "Are you defining a method called `{}`?".format(
        test_method
    )
    ...
    my_class_arguments = (
        my_class.def_args_(test_method)
        .match(
            {
                "type": "FunctionDef",
                "name": "_convert_data",
    ...
    assert my_class_arguments, """Are you defining a method `{0}` for the `{1}` class?
        Are you declaring the correct name and number of parameters?""".format(
    ...
    test_code = (
        my_method.assign_()
        .match({"targets_0_type": "Name", "targets_0_id": "recs", "value_type": "List"})
        .exists()
    )
    assert (
        test_code
...
def test_humidity_convert_loop_module4(parse):
    ...
    my_class = my_file.query("class {0}({1}): ??".format(test_class, parent_class))
    assert my_class.exists(), """Have you created a class called `{0}`?
        Is your class inheriting the properties of the `{1}` class?""".format(
        test_class, parent_class
    )
    ...
    assert test_code, """Do you have a `for` loop, looping through `data`? 
    ...
    assert test_code, """Inside your loop, are you converting `rec` value to `float()` and multiplying it by `100`?

        Are you appending it to `recs` list?"""

    test_code = (
        my_method.returns_call()
        .match({"type": "Return", "value_type": "Name", "value_id": "recs"})
        .exists()
    )
    assert test_code, """Are you returning `recs` list from the `{}` method?""".format(
        test_method
    )
...
def test_humidity_by_area_method_module4(parse):
    ...
    assert test_code, """Are you creating a variable called `recs` set equal to 
        the `{}` method from the `{}` parent class?
        Are you passing `"humidity"` as the first argument to the method call?
        Are you passing `rec_area` as the second argument to the method call?""".format(
        test_method, parent_class
    )
    ...
    assert test_code, """Are you returning a call from the `{}` method?
        Are you calling the `_convert_data` method?
        Passing `recs` as the only argument?""".format(
        test_method
    )
...
def test_humidity_by_date_method_module4(parse):
    ...
    my_file_import = my_file.from_imports("datetime", "date")
    assert my_file_import, "Are you importing `date` from datetime in `{}`".format(
        test_file
    )
    ...
    assert test_code, """Are you creating a variable called `recs` set equal to 
        the `{}` method from the `{}` parent class?
        Are you passing `"humidity"` as the first argument to the method call?
        Are you passing `rec_date` as the second argument to the method call?""".format(
        test_method, parent_class
    )
...
def test_sensor_app_temp_info_by_area_module4(parse):
    ...
    my_file_import = my_file.from_imports("humidity_info", "HumidityData")
    assert (
        my_file_import
    ), "Are you importing `HumidityData` from `humidity_info` in `{}`".format(test_file)
    ...
    assert test_code, """Are you creating an instance of the '{}' class called `humidity_data`?
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_area` from the `humidity_data` object?
        Are you passing `rec_area=test_area` as the only argument to the method?
        """

    my_file_import = my_file.from_imports("statistics", "mean")
    assert my_file_import, "Are you importing `mean` from `statistics` in `{}`".format(
        test_file
    )
...
def test_sensor_app_temp_info_by_date_module4(parse):
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_date` from the `humidity_data` object?
...
def test_particle_convert_loop_module4(parse):
    ...
    assert test_code, """Inside your loop, are you converting `rec` value to `float()`
        Are you appending it to `recs` list?"""
...
def test_particle_get_concentration_method_module4(parse):
    ...
    assert test_code, """Are you creating a dictionary called `particulate`?
...
def test_particle_get_concentration_for_module4(parse):
    ...
    assert test_code, """Are you creating a loop over `data`?
        Are you using `rec` as the iterator?"""

    test_code = (
        my_method.returns_call()
        .match({"type": "Return", "value_type": "Name", "value_id": "particulate"})
        .exists()
    )
    assert test_code, """Are you returning `particulate` from the method?"""
...
def test_sensor_app_particle_info_by_area_module4(parse):
    ...
    my_file_import = my_file.from_imports("particle_count_info", "ParticleData")
    assert (
        my_file_import
    ), "Are you importing `ParticleData` from `particle_count_info` in `{}`".format(
        test_file
    )
    ...
    assert test_code, """Are you creating an instance of the '{}' class called `particle_data`?
        Are you passing `data` as the initialization argument for the constructor?
        """.format(
        test_class
    )
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_area` from the `particle_data` object?
        Are you passing `rec_area=test_area` as the only argument to the method?
        """
    ...
    assert test_code, """Are you setting `concentrations` to the method call `get_data_concentrations` from the `particle_data` object?
...
def test_sensor_app_particle_info_by_date_module4(parse):
    ...
    assert test_code, """Are you setting `recs` to the method call `get_data_by_date` from the `particle_data` object?
        Are you passing `rec_date=test_date` as the only argument to the method?
        """
